...
def is_allowed_year(year: str) -> bool:
    return str(year).strip() in ALLOWED_YEARS


def ensure_allowed_year(year: str, *, arg_name: str) -> str:
    year = str(year).strip()
    if not is_allowed_year(year):
        allowed = ", ".join(sorted(ALLOWED_YEARS, reverse=True))
        print(f"ERROR: {arg_name} supports only years: {allowed}", file=sys.stderr)
        sys.exit(1)
    return year
...
def cve_path(cve: str) -> Path:
    year = cve.split("-")[1]
    ensure_allowed_year(year, arg_name="CVE year")
...
def extract_cves_from_repo(repo: dict, year: str) -> set[str]:
    year = ensure_allowed_year(year, arg_name="discovery year")
    cves: set[str] = set()
...
def main() -> None:
    ...
    ap.add_argument("--cve", help="Process single CVE. Supported years: 2025, 2026 (e.g. CVE-2025-0001)")
    ...
    ap.add_argument("--discover-year", help="Discover CVEs for a given year from GitHub. Supported years: 2025, 2026")
    ...
    if args.discover_year:
        ...
        if not re.fullmatch(r"\d{4}", year):
            ...
            sys.exit(1)
        year = ensure_allowed_year(year, arg_name="--discover-year")
    ...
    m = CVE_RE.fullmatch(cve)
    if not m:
        print("ERROR: --cve must look like CVE-YYYY-NNNN", file=sys.stderr)
        sys.exit(1)
    ensure_allowed_year(m.group(1), arg_name="--cve year")
